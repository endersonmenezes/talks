...
em container (ver compose-tests.yaml) — independe da máquina do professor.
...
APP_DIR = pathlib.Path(
    os.environ.get(
        "APP_DIR",
        pathlib.Path(__file__).resolve().parent.parent / "example-generated-code" / "app",
    )
)
...
class TestRequisitoOculto:
    def test_c1_cliente_tem_telefone(self):
        """O ENUNCIADO (example-problem.md) exige cliente com id, nome e telefone.
