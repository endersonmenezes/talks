...
O `Dockerfile.example` compila e roda o `DebugExample.java` — o mesmo arquivo com
...
# Podman
podman-compose -f compose-example.yaml build
```

Saída: o erro de compilação aparece no log do build — exatamente como na jornada
do `answer_key.md`, só que dentro do ciclo de build da imagem.
