---
title: "Avaliação 04 — Prova Prática"
type: evaluation
status: active
area: career
...
(categorias A–D, executada em container — ver `exemplo-testes/`), mais a qualidade
...
- `carreira-01-sdd/` — problema de exemplo + mini-resposta-modelo (os 5 `.md`) + **exemplo de correção multi-agentica executada**: `exemplo-codigo-gerado/` (janelas isoladas → app consolidado + PROVENIENCIA.md) e `exemplo-testes/` (suíte do professor A/B/C/D, com compose Docker/Podman — resultado real: 16 passaram, 1 falha proposital de requisito oculto)
- `carreira-02-debug/` — arquivo Java minúsculo com 5 erros propositais + gabarito com jornada de debug verificada + exemplo de Container/Compose
- `carreira-03-crud/` — contrato de exemplo + suíte pública mini (pytest)
