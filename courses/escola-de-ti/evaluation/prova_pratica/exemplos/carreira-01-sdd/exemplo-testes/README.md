...
janela-constitution/  janela-spec/  janela-plan/  janela-tests/
        │
        ▼  ② consolidação (assembly) — resolve conflitos entre janelas
exemplo-codigo-gerado/app/   (+ PROVENIENCIA.md: de onde veio cada decisão)
        │
        ▼  ③ suíte do professor, EM CONTAINER (docker/podman) — não depende da máquina do professor
suite_professor.py → relatório A/B/C/D
```

### Por que container na correção?

`compose-testes.yaml` sobe o app do aluno + a suíte em containers. O resultado da correção
não depende do que está instalado na máquina do professor — qualquer ambiente com
Docker/Podman reproduz a mesma correção.

```bash
# Docker
docker compose -f compose-testes.yaml up --build --abort-on-container-exit
# Podman
podman-compose -f compose-testes.yaml up --build --abort-on-container-exit
```
...
### 🔀 Conflitos reais entre janelas (documentados em `../exemplo-codigo-gerado/app/PROVENIENCIA.md`)

- **Idioma dos endpoints**: janela-tests criou `/quadras` e `/reservas`; spec/constitution usavam `/courts`, `/bookings`. O contrato venceu.
- **Regras de duração**: janela-plan gerou 30min–4h; spec manda 1h–3h. A spec venceu.
- **Status codes**: janela-plan mapeava tudo para 422; a spec exige 400/404/409 distintos.
- **IDs**: uuid (janela-spec) vs. inteiros sequenciais (janela-plan) — prevaleceu o plano.
